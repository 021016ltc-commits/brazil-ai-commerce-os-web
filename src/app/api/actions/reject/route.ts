...
export async function POST(request: Request) {
  ...
  if (!isDecisionInput(body)) {
    return NextResponse.json(
      {
        error: "Invalid rejection payload. action_id and actor_role are required.",
      },
      { status: 400 },
    );
  }
  ...
  try {
    const result = await withTenant(tenantId, () => dataService.rejectAction(body));
    return NextResponse.json({ tenant_id: tenantId, ...result });
  } catch (error) {
    logApiError("/api/actions/reject", error);
    return NextResponse.json(
